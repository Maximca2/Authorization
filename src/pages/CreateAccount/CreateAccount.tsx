...
//helpers
import { toastError, toastSuccess, toastInputIsEmpty } from "../../helpers";
...
export function checkIfCurrentAccountExist(value: boolean) {
  if (!value) {
    return toastError(accountAlreadyExist, options);
  }
  setTimeout(() => {
    return toastSuccess(successCreateAccount, options);
  }, 500);
}

const CreateAccount = () => {
  const dispatch = useDispatch();
  const [dataToLogIn, setDataToLogIn] = useState<CurAccountData>({
    lastName: "",
    name: "",
    password: "",
    nicName: "",
  ...
  const createAccount = (obj: CurAccountData) => {
    if (checkAllValueInObject(obj)) {
      ...
      toastInputIsEmpty(fillInputs, options);
    }
  };
